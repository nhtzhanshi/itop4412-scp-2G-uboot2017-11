...
#endif
	printascii("cp uboot successfully\n");
}
...
#define POWER_ADR (*(volatile unsigned int *)0x1002330C)
...
//	POWER_ADR = 0x00005300;		
	tmp = POWER_ADR;
    
	printhex8((unsigned int)power->ps_hold_control);
	printascii("\n");	
    printhex8((unsigned int)&power->ps_hold_control);
	printascii("\n");	
    printhex8((unsigned int)power);
	printascii("\n");	
    printhex8((unsigned int)&power);
	printascii("\n");	
	printhex8(tmp);
	...
	//writel(0x00005300, 0x1002330C);
	//LED2_ON;
	copy_uboot_to_ram();

	printascii("uboot ready to run\n");
